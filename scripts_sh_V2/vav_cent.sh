...
#SBATCH --job-name="train_vav_CENTRALIZED_V2"
#SBATCH --nodes=1
...
#SBATCH --output=./scripts_sh_V2/output/vav/std_%x_%j.txt
#SBATCH --error=./scripts_sh_V2/output/vav/err_%x_%j.txt
...
name_hdf5_file=$(toml get -r ${path_dataset_config} name_hdf5_file)
path_hdf5_file="${path_data}/${name_hdf5_file}"

n_samples=$(get-hdf5-dataset-length --path_hdf5_file ${path_hdf5_file})
...
python ./scripts_python/training/update_training_config.py\
	...
	--path_to_save_model="model_weights/vav/${seed}_${SLURM_JOB_ID}/"\
	--seed="${seed}"\
	--no-fl_training\
	--no-vgg_training\
	--no-swin_training\
...
python ./scripts_python/training/vav_centralized_V2.py\
	--path_src="${path_src}"\
