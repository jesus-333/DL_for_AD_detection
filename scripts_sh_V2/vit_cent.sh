...
#SBATCH --job-name="train_vit_CENTRALIZED_V2"
#SBATCH --nodes=1
...
#SBATCH --output=./scripts_sh_V2/output/vit_transformer/std_%x_%j.txt
#SBATCH --error=./scripts_sh_V2/output/vit_transformer/err_%x_%j.txt
...
srun python ./scripts_python/training/update_training_config.py\
	...
	--path_to_save_model="model_weights/vit/${seed}_${SLURM_JOB_ID}/"\
	--seed="${seed}"\
	--no-fl_training\
	--no-vgg_training\
	--no-swin_training\
...
srun python ./scripts_python/training/vit_transformer_centralized_V2.py\
	--path_src="${path_src}"\
