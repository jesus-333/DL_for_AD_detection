...
percentage_data_used_for_training=-1
n_folds=5
seed=42
n_repetitions=1
...
for repetition in $(seq 1 $n_repetitions); do
	...
	# For each repetition, perform n_folds cross validation
	for current_fold in $(seq 1 $n_folds); do
		echo -e "\t-------------------------------------"
		echo -e "\tFOLD ${current_fold} / ${n_folds}"
	...
	done # End of the for loop for cross validation folds
