...
def nii_to_hdf5() -> None:
    ...
    parser = argparse.ArgumentParser(prog = "nii-to-hdf5", derscription = "Convert all nii files in a folder to a single hdf5 files.")
    ...
    parser.add_argument("--labels_file"   , default = None, help = "Path to the file with the labels. This file must be a csv file with at least two columns 'file_name' (i.e. the name of the file with extension) and 'labels_int' (the label of the sample contained in the file, stored as an int). An optional third columns, 'labels_str' coulb be present (the label of the sample contained in the file, stored as an str). During the conversion, the labels will be extracted from this file by matching the 'file_name' column with the name of the nii files. If passed together with hd5f file,  it will created also a 'dataset_info.csv' file in the output folder, which will contain for each sample in the hdf5 file the corresponding label, extracted from the labels file. If not specified, no labels will be extracted and the 'dataset_info.csv' file will not be created.")
