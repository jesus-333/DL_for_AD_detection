...
- train_idx.npy : indices of the training data (all clients) (basically the concatenation of all train_idx_client_{i}.npy files)
...
- path_to_save/
    ...
    - train_idx.npy             (80% of the data)

If use_cross_validation is set to True, the training data will be split in n_folds folds for cross validation. In this case, percentage_data_used_for_training will be rounded to the closest standard value (0.5, 0.75, 0.8, 0.9, 0.95, 0.98) if it is not one of them.
The number of folds (n_folds) will be computed as n_folds = 1 / (1 - percentage_data_used_for_training).
E.g. if percentage_data_used_for_training is 0.8, the number of folds will be 5 (1 / (1 - 0.8) = 1 / 0.2 = 5).
In case of cross validation, the data will be saved in subfolders named fold_0, fold_1, ..., fold_{n_folds - 1}.
Inside each folder, the files will be named as described before.
Since the n_of_folds is computed from percentage_data_used_for_training, only some specific values are allowed for the arguments (i.e. values that give an integer number of folds from the formula n_folds = 1 / (1 - percentage_data_used_for_training)).

E.g. if n_client is 3, percentage_data_used_for_training is 0.75 and use_cross_validation is True, the folder will contain the following files:
- path_to_save/
    - fold_0/
        - train_idx_client_0.npy    (data for client 0, roughly 26.67% of the data)
        - train_idx_client_1.npy    (data for client 1, roughly 26.67% of the data)
...
# Boolean arguments
parser.add_argument('--use_cross_validation'             , default = None, action = 'store_true', help = 'If passed the script will use cross validation. See the script header for more details.')
...
if args.use_cross_validation is not None              : use_cross_validation = args.use_cross_validation
...
if percentage_data_used_for_training not in possible_values_percentage_data_used_for_training and use_cross_validation :
    print(f"Warning: percentage_data_used_for_training is set to {percentage_data_used_for_training}. If you enable cross validation, it is recommended to use one of the standard values: {possible_values_percentage_data_used_for_training}")
...
def split_and_save_indices(idx_train, idx_val, labels_train, path_to_save : str, print_info : bool = True) :
    ...
    The function was created to avoid code duplication when using cross validation.
    """
    ...
    np.save(f'{path_to_save}train_idx.npy', idx_train)
...
if use_cross_validation :
    ...
    n_folds = int(1 / (1 - percentage_data_used_for_training))
