"""
Variation of `ADNI_extract_middle_slices` created to work with Nifti (nii) files.
...
dataset_name = "ADNI_MRI_Normalized"
path_data = f"./data/{dataset_name}/"
path_dataset_info = f"./data/{dataset_name}/data_info.json"

path_to_save = './data/ADNI_MRI_Normalized_axial_middle_slice/'

conversion_type = 1 # 0 : create a file for each slice, 1 : create a single tensor with all the slices, 2 : create both the previous types
...
list_of_files = support_dataset.get_all_files_from_path(path_data, filetype_filter = 'nii')
...
for i in range(len(list_of_files)) :
    ...
    sample = torch.from_numpy(img.get_fdata()).float()
    sample = (sample - torch.min(sample)) / (torch.max(sample) - torch.min(sample))

    # Get z_matrix (n. of slices)
    z_matrix = sample.shape[2]
    ...
    for idx in indices :
        ...
        tmp_info_csv.append(dataset_name)
        ...
        tmp_info_dict['original_dataset'] = dataset_name 
