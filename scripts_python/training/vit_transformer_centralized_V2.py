...
if model_config['pretrained'] : # Get directly the moodel and the preprocess functions from the timm library
    model, preprocess_functions = vit.get_vit(model_config)
else : # Get only the model from the timm library and create the preprocess functions with the get_preprocess_functions function in support_model.py

    # Create the model
    model = vit.timm_vit(config = model_config)
    ...
    # Yeah, I know that the function get_vit if 'pretained' is False does not return None for the preprocess_functions (and I can use them directly)
    # But I prefer to have this section here in case I need a more "custom" approach in the future.
